        ...
        try (Connection conn = new DBConnection().getConnection()) {
            ...
            if (!isTableAvailable(conn, requestedTableId, arrival, departure)) {
                ...
                if (available.isEmpty()) {
                    message = "The table " + requestedTableId + " is not available. No tables are available for this period.";
                } else {
                    String numbers = available.stream()
                            .map(t -> String.valueOf(t.getNumber()))
                            .collect(Collectors.joining(", "));
                    message = "The table " + requestedTableId + " is not available. Available tables: " + numbers;
                }
                conn.rollback();
            ...
            throw new RuntimeException("Error during backup : " + e.getMessage(), e);
        }
    }
